...
export async function getProductByIdDSP(product_id: string) {
  const { data } = await apiDSP.get(`/products/supplier/detail/${product_id}`);
  return data as ProductItem;
}

export async function CreateProductDSP(input: ProductInput) {
  const { data } = await apiDSP.post("/products/supplier", input, {
    headers: {
...
export async function deleteProductDSP(product_id: string) {
  const { data } = await apiDSP.delete(`/products/supplier/${product_id}`);
  return data;
}

export async function editProductDSP(input: ProductInput, product_id: string) {
  const { data } = await apiDSP.put(`/products/supplier/${product_id}`, input, {
    headers: {
      "Content-Type": "application/json",
    },
  });

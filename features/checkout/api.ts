...
export type SearchBy = "order" | "product" | "shipping_address";
...
export async function getCheckOutMain(params?: GetAllCheckoutParams) {
  const { data } = await apiAdmin.get("/checkout/main-checkouts", {
    params: {
      ...
      ...(params?.to_date !== undefined && { to_date: params.to_date }),
      ...(params?.search !== undefined && { search: params.search }),
      ...(params?.search_by !== undefined && { search_by: params.search_by }),
...
export async function updateNoteForMainCheckout(
  main_checkout_id: string,
  note: string,
) {
  const { data } = await apiAdmin.put(
    `/checkout/note-for-main-checkout/${main_checkout_id}`,
    null,
    {
      params: { note },
    },

...
export interface DpdOutboundParcelData {
  weight: number;
  content: string;
  outbound_id: string;
...
export interface DpdOutboundOrderDataItem {
  shipping_address: DpdOutboundShippingAddress;
  parcel_data: DpdOutboundParcelData;
}

export interface CreateDpdOutboundLabelsPayload {
  orderdata: DpdOutboundOrderDataItem[];
  label_size: "A6" | string;
  dpd_shipping_date: string;
  dpd_label_position: "UpperLeft" | string;
}

...
export async function createPayment(paymentInput: PaymentFormValues) {
  const { data } = await api.post(`/payments/`, paymentInput, {
    headers: {
...
export async function capturePayment(paypal_order_id: string) {
  const { data } = await api.post(`/payments/${paypal_order_id}/capture`, {
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${localStorage.getItem("access_token")}`,
    },
...
export async function getPaymentStatus(payment_id: string) {
  const { data } = await api.get(`/payments/${payment_id}/capture`);
  return data;
}

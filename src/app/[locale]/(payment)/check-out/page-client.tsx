...
export default function CheckOutPageClient() {
  ...
  const { data: invoiceAddress } = useQuery({
    ...
    queryFn: () =>
      getInvoiceAddressByUserId(userIdLogin ? userIdLogin : userId),
    retry: false,
    enabled: !!(userIdLogin ? userIdLogin : userId),
  });
  ...
  const normalizedItems = normalizeCartItems(
    hasServerCart ? cartItems.flatMap((group) => group.items) : localCart,
    hasServerCart
  );
                  ...
                  <span className="text-right col-span-2">
                    ...
                    cartItems.length > 0
                      ? cartItems
                          ...
                          .reduce(
                            (total, item) => total + (item.final_price ?? 0),
                            0
                          )
                      : localCart
                          ?.filter((item) => item.is_active)

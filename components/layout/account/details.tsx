...
const AccountDetails = ({ user }: AccountDetailsProps) => {
    ...
    const [isNotified, setIsNotified] = useState<boolean>(user.is_notified ?? false)
                    ...
                    <Card className="shadow-sm rounded-none border-none relative">
                        ...
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div className="flex items-center gap-4">
                                <User2 className="w-7 h-7" strokeWidth={1} stroke='black' />
                                <CardTitle className='text-2xl font-light'>{t('personal_data')}</CardTitle>
                            </div>
                            <Dialog open={openUserDialog} onOpenChange={setOpenUserDialog}>
                                <PenBoxIcon className="w-7 h-7 cursor-pointer" strokeWidth={1} onClick={() => setOpenUserDialog(true)} />
                                <DialogContent className='lg:w-[800px] space-y-6'>
                                    <DialogHeader>
                                        <DialogTitle className='mb-4'>{t('edit_user_data')}</DialogTitle>
                                        <EditUserDialog setOpen={setOpenUserDialog} open={openUserDialog} currentUser={user} />
                                    </DialogHeader>
                                </DialogContent>
                            </Dialog>
                        </CardHeader>
                        <CardContent>
                            <p className="font-medium text-lg text-black">{user.first_name} {user.last_name}</p>
                    ...
                    <Card className="shadow-sm rounded-none border-none relative">
                        ...
                        <CardHeader className="flex flex-row items-center justify-between">
                            ...
                            <Dialog open={openAddressDialog} onOpenChange={setOpenAddressDialog}>
                                <PlusSquare className="w-7 h-7 cursor-pointer" strokeWidth={1} onClick={() => setOpenAddressDialog(true)} />
                    ...
                    <Card className="shadow-sm rounded-none border-none relative">
                        ...
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div className="flex items-center gap-4">
                                <HomeIcon className="w-7 h-7" strokeWidth={1} stroke='black' />
                                <div>
                                    <CardTitle className='text-2xl font-light'>{t('invoiceAddress')}</CardTitle>
                                </div>
                            </div>
                            <Dialog open={openAddressDialog} onOpenChange={setOpenAddressDialog}>
                                {invoiceAddress ? ''
            ...
            <div className='col-span-2 lg:col-span-1 w-full lg:space-y-8 space-y-4'>
                ...
                <div>
                    <Link href={`/my-order`} className="">
                        <Card className="shadow-sm rounded-none border-none relative">
                            ...
                            <CardHeader className="flex flex-row items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <BoxIcon className="w-7 h-7" strokeWidth={1} stroke='black' />
                                    <CardTitle className='text-2xl font-light'>{t('orderHistory')}</CardTitle>
                                </div>
                                <LucideEye className="w-7 h-7 cursor-pointer" strokeWidth={1} />
                            </CardHeader>
                        </Card>
                    </Link>
                </div>
                ...
                <div>
                    <Link href={`/wishlist`} className="">
                        <Card className="shadow-sm rounded-none border-none relative">
                            ...
                            <CardHeader className="flex flex-row items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <Heart className="w-7 h-7" strokeWidth={1} stroke='black' />
                                    <CardTitle className='text-2xl font-light'>{t('wishlist')}</CardTitle>
                                </div>
                                <LucideEye className="w-7 h-7 cursor-pointer" strokeWidth={1} />
                            </CardHeader>
                        </Card>
                ...
                <div className="">
                    <Card className="shadow-sm rounded-none border-none relative">
                        ...
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div className="flex items-center gap-4">
                                <Mails className="w-7 h-7" strokeWidth={1} stroke='black' />
                                <CardTitle className='text-2xl font-light'>{t('newsletter')}</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent>
                            <div className='space-y-6'>
                                ...
                                <div className="flex items-start space-x-3">
                                    ...
                                    />
                                    <Label htmlFor="newsletter" className="text-lg leading-snug font-light">
                                        {t('newsletter_subscribe_confirm', { email: user.email })}
                                    </Label>
                                </div>

                                <Button variant={'outline'} onClick={() => handleChange()} className="w-full text-lg rounded-none py-2 h-fit border-black lg:mt-4">ANMELDEN</Button>
                            </div>
                        </CardContent>
                    </Card>
                </div>

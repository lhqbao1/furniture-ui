...
const AnimatedCarousel = () => {
  ...
  return (
    <div className="flex flex-col section-padding">
      <h2 className="section-header mb-6">{t("categories")}</h2>
      {!categories || isLoading || isError ? (
        <AnimatedCarouselSkeleton />
      ) : (
        <div
          className="relative w-full xl:h-[600px] h-[300px] xl:min-h-[500px]"
          style={{
            perspective: 2000,
        ...
        >
          {categories.slice(0, 8).map((item, index) => {
            ...
            const depth = Math.abs(normalizeAngle(angle - Math.PI / 2));

            const rotateY = Math.sin(angle) * 25; // nghiêng trái / phải
            const rotateX = -Math.cos(angle) * 10; // hơi đổ xuống
            const blur = depth > 0.3 ? 2 : 0;
            const brightness = 1 - depth * 0.3;

            const isCenter =
              Math.abs(normalizeAngle(angle - Math.PI / 2)) < 0.02;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  transform: `
    translate3d(${x}px, ${y}px, ${z + zOffset}px)
    rotateY(${rotateY}deg)
    rotateX(${rotateX}deg)

...
export default function CommentImageDialog({
  listComments,
  comment,
}: {
  listComments: ReviewResponse[];
  comment: ReviewResponse;
}) {
  // Lọc chỉ những comment có static_files
  const commentsWithImages = listComments.filter(
    (c) => c.static_files && c.static_files.length > 0,
  );
              ...
              <DialogTrigger asChild>
                ...
                >
                  <Image
                    ...
                    width={80}
                    height={80}
                    className="rounded-xl object-contain min-h-16"
                    unoptimized
                  />
                  {isLast && (

...
const IconList = ({ currentProduct }: IconListProps) => {
  ...
  useEffect(() => {
    containersRef.current.forEach((container) => {
      ...
      const textElements =
        container.querySelectorAll<HTMLElement>(".button-name");
      if (!icon || textElements.length === 0) return;

      gsap.set(icon, { position: "absolute", right: "10px" });
      textElements.forEach((t) =>
        gsap.set(t, { position: "absolute", right: "55px" }),
      );
    ...
    <div className="flex flex-col gap-2 items-end">
      {items.map((item, idx) => (
        <div
          ...
          key={idx}
          ref={(el) => {
            if (el) containersRef.current[idx] = el;
          }}
        ...
        >
          <p className="text-white text-xs font-medium whitespace-nowrap button-name">
            {item.label}
          </p>
          <item.Icon
            className="text-white button-icon"
            size={18}
          />
        </div>
      ))}
